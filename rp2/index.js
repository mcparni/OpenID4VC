...
const app = express()
const port = 3005
const cookieName = `RP2OpenIDforVC`;
const os = require('os');
const fs = require('fs');
...
const URI = `https://relying-party2.com:${port}`;
const OP_URI = `https://openid-provider.com:3000`;
const WALLET_URI = `https://siop-wallet.com:3002`;

const ID = "did:ion:EiCj4GOM4qFQE7FmmKE6f_pLrMEbeXbmbI6Ly3ckBL8QeQ";
// Relying Party 2 DID: did:ion:EiCj4GOM4qFQE7FmmKE6f_pLrMEbeXbmbI6Ly3ckBL8QeQ
...
app.get('/login', (req, res) => {
    ...
    const clientID = `AnotheridCLIENT`;
...
app.get('/walletlogin', async (req, res) => {
  ...
  const challenge = base64URLEncode(sha256(verifier));
  req.session.verifier = verifier;
  let authz = `${authzURL}?request=${requestObj}`;
  console.log(`Authorization request to: ${authz}`);
  console.log(`Code challenge: ${challenge}`);
  console.log(`Code verifier: ${verifier}`);
  res.redirect(authz);
...
app.get('/', (req, res) => {
    ...
    let html = `<div style="width: auto; padding: 10px; background-color:#9dd493; font-size: 0.6em; color: #000;">
    <h1>Relying Party2 -- did:ion:EiBQsxvT1tz0Cz7KEfFuJhJt_134d_suJlwZ3S_bXVnoBA</h1>
  </div>`;
    if(error) {
      html += `${errorMsg}`;
    }
...
app.get('/redirect', (req, res, next) => {
  ...
  let sec = Buffer.from("AnotheridCLIENT:abc12").toString('base64');
  axios.post(`${OP_URI}/token`, {
    client_id: 'AnotheridCLIENT',
...
app.get('/redirect', (req, res) => {
  console.log("vp_token present, handling differently");
  // req.query.error
  ...
  if(!isTrustedIssuer(vcBody.iss)) {
    return ctx.redirect(`/?error=Issuer Error&error_description=The Verifiable Credential is from untrusted issuer`)
  }
