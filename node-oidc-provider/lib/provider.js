...
function cookieParser(cookieString, key) {
  if (cookieString === "")
      return {};
  ...
  for(let i = 0; i < splittedPairs.length; i++) {
    for(let j = 0; j < splittedPairs[i].length; j++) {
      if(splittedPairs[i][0] == key && splittedPairs[i][j].length > 1) {
        return splittedPairs[i][1];
      }
    } 
  }
  return cookieObj;
}
function inArrayAt(arr, str) {
...
async function getInteraction(req, res) {
  ...
  let resCookies = ctx.response.header['set-cookie'];
  let interationCookie = undefined;
  try {
    interationCookie = inArrayAt(resCookies, "interaction=");
    if(interationCookie.length > 0) {
      interationCookie = cookieParser(interationCookie, "interaction");
    }
  } catch(e) {
    //console.log("error")
  }
  let id;
  if(interationCookie != undefined) {
    id = interationCookie;
  } else {
    id = ssHandler.get(
      ctx.cookies,
